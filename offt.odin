...
fft_execute_inplace :: proc(plan: ^FFTPlan, buf: []Complex32) {
    n := plan.n;
    if plan.log2_n < 2 {
...
// -------- FFT with separate input/output buffers --------

// FFT that copies from input buffer to output buffer and performs FFT on output
fft_execute :: proc(plan: ^FFTPlan, input: []Complex32, output: []Complex32) {
    assert(len(input) == plan.n, "input buffer length != plan size");
    assert(len(output) == plan.n, "output buffer length != plan size");
    
    // Copy input to output
    copy(output, input);
    
    // Perform in-place FFT on output buffer
    fft_execute_inplace(plan, output);
}
